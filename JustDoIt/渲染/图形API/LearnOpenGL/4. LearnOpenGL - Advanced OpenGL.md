...
> 从OpenGL 4.2版本起，也可以添加一个布局标识符，显式地将Uniform块的绑定点储存在着色器中，这样就不用再调用 `glGetUniformBlockIndex` 和 `glUniformBlockBinding` 了。下面的代码显式地设置了Lights Uniform块的绑定点。
> `layout(std140, binding = 2) uniform Lights { ... };`

==注意⚠️：我们需要对**每个**着色器重复这一步骤。==
