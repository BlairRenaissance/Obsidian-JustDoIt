...
==`dll 文件`==：Windows 平台下编译出的动态库的二进制格式。 [[2.2 编译器 Compiler#二进制文件]]
...
==`gcc`==：GNU项目中开发的针对C语言的编译器。[[2.5 工具链 Toolchain#GNU 工具链]]
...
| **平台/语言**                            | **文件类型**  | **Debug 版本 (开发阶段)**                                                | **Release 版本 (线上/发布阶段)**                                                   | **Crash 还原核心文件**                |
| ------------------------------------ | --------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------- |
| **Android**<br><br>(C/C++)           | Native 符号 | **嵌入式符号**<br><br>编译出的 `.so` 文件通常直接包含调试信息，体积较大。                     | **符号被剥离 (Stripped)**<br><br>为了减小体积，`.so` 中的符号表被剔除。APP 中运行的是无符号的 `.so`。     | **带符号表的 .so**<br><br>(需匹配 UUID) |
| **Android**<br><br>(Java/Kotlin)     | 代码混淆      | **无混淆**<br><br>类名、变量名保持源码原样，堆栈直接可读。                                | **开启混淆 (R8/ProGuard)**<br><br>类名变成 `a.b.c`，代码被优化和压缩。                       | **mapping.txt**                 |
| **iOS / macOS**<br><br>(Obj-C/Swift) | 调试符号      | **DWARF in Mach-O**<br><br>调试符号通常直接嵌入在二进制文件中（或者生成临时的 dSYM），可以直接调试。 | **符号被剥离 -> dSYM**<br><br>二进制文件中的符号被“抽”出来，生成独立的 `.dSYM` 包。APP 本体不含符号。       | **.dSYM**<br><br>(需匹配 UUID)     |
| **Windows**<br><br>(C++/C#)          | 调试数据库     | **Full PDB**<br><br>生成包含完整私有符号、行号信息的 PDB，通常直接链接。                   | **Strip PDB / Separate**<br><br>exe/dll 不含调试信息。通常会生成独立的 PDB，有时会剥离私有符号以防泄露。 | **.pdb**<br><br>(需匹配签名)         |
