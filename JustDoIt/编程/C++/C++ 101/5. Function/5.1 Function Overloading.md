...
如果没有找到完全匹配，编译器将尝试通过对参数应用数字提升来找到匹配。[[3.5 隐式类型转换 Implicit Type Conversion#3. 数值提升（Numeric promotions）]]
...
[[3.5 隐式类型转换 Implicit Type Conversion#4. 数值转换（Numeric conversions）]]
