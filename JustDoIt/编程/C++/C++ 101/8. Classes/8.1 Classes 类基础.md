...
### Inline member functions 内联成员函数

在类定义内部定义的函数会被隐式地视为内联（inline）函数，这样它们就可以被 `#include` 到多个代码文件中，而不会违反 ODR（One Definition Rule，一处定义规则）。

在类定义外部定义的函数则不会被隐式地视为内联函数。如果需要将它们设为内联函数，可以使用 `inline` 关键字。

### 默认参数

对于非成员函数的默认参数：如果函数有前置声明（尤其是在头文件中），就把默认参数写在前置声明那里。否则，就把默认参数写在函数定义里。

由于成员函数总是在类定义中声明（或定义），所以成员函数的默认参数最佳实践其实更简单：**始终把默认参数写在类定义内部。**

以此来尽可能保障默认参数的可见性。

> [!Best practice]
> 把所有默认参数都写在类定义内部（即头文件里）。
