...
与函数模板类似，类模板通常定义在头文件中，以便可以将其包含到任何需要它们的代码文件中。

模板定义和类型定义均不受单一定义规则的约束（只要各个文件中的定义是一致的）。

[[5.6 Using function templates in multiple files#为什么把模板函数放在头文件里没有违反单一定义规则 (ODR) ？]]
...
这样给Pair起个叫Coord的别名有什么意义呢？直接不叫 Pair 叫 Coord 不就行了？
