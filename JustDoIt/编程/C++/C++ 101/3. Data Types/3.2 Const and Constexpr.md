...
为什么只有 const **integral** variables with a constant expression initializer 被视为 constant expression，而 const non-integral variables 在任何情况下都不是呢？

在 C 和 C++ 中，数组对象的长度需要是个编译期可求值的数值。

在 C 语言中，这可以通过 preprocessor macro 或者枚举常量来实现，但不能通过 const 变量（不包括变长数组，变长数组有其他缺点）。

为了改善这种情况，C++ 想要允许使用 const 变量来代替宏。但是，变量的值通常被认为仅在运行时才可知，这使得它们无法用作数组长度。

为了解决这个问题，C++ 语言标准增加了一个【豁免】，以便具有常量表达式初始化器的 const 整型将被视为编译时已知的值，从而可用作数组长度。

所以当 C++11 引入常量表达式时，将带有常量表达式初始化器的 const int 保留到该定义中是合理的。委员会曾讨论过是否也应包含其他类型，但最终决定不这样做。
...
---
## Constexpr function
https://www.learncpp.com/cpp-tutorial/constexpr-functions/#google_vignette
