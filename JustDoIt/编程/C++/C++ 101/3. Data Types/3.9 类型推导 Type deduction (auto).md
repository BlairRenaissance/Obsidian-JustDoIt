...
这在函数的返回类型多变时很有用，显式指定返回类型就必须手动更新所有相关的返回类型声明：
...
> [!Warning]
> 使用自动返回类型的函数必须先完全定义才能使用。仅有前向声明是不够的。

很合理，因为只有前向声明，编译器没有任何函数信息。
...
在这种情况下，“auto”不执行类型推断——它只是使用尾随返回类型的语法的一部分。
...
// decltype 是 C++11 引入的一个类型说明符，用于推导表达式的类型。
```
