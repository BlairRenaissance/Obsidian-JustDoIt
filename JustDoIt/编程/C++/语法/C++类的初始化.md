...
| 位域         | 必须通过初始化列表初始化                  |
* 初始化列表在构造函数体执行之前被处理。引用成员必须通过初始化列表进行初始化，这是因为引用必须在其创建时被绑定到一个有效的对象，并且在其生命周期内不能被重新绑定。C++语言规范要求引用在声明时必须被初始化。这是为了确保引用在任何时候都指向一个有效的对象或变量。编译器会强制执行这一规则，以防止引用未初始化或指向无效对象。
